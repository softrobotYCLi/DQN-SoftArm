    ...
    def _build_net(self):
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer, trainable):
            ...
            with tf.variable_scope('l4'):
                w4 = tf.get_variable('w4', [n_l1, self.n_actions], initializer=w_initializer, collections=c_names,  trainable=trainable)
...
with tf.variable_scope('DQN_with_prioritized_replay'):
    if MODE == False:
        RL_prio = DQNPrioritizedReplay(
            n_actions=4, n_features=24, memory_size=20000,
...
def train(RL):
    MAX_STEP = 5000
    LRARN_START = MAX_STEP*0.05
    EPSILON = 0.3
    EPSILON_INCREMENT = (1-EPSILON)/MAX_STEP
    win = 0
    display_step = MAX_STEP-20
    win_rate_ = np.zeros([MAX_STEP,1])
    for i_episode in range(MAX_STEP):
        # small_epoch = 0 #记录小循环运行次数的mark
        ...
        while not terminal:
            ...
            # temp_memory.extend(np.array([observation, action, reward, observation_]))
        ...
        # RL.store_transition(temp_memory[4*i],temp_memory[4*i+1],award*temp_memory[4*i+2],temp_memory[4*i+3])
        ...
        win_rate_[i_episode,0] = win/(i_episode+1)*100
        print("WIN: {:03d}/{:03d} ({:.3f}%)".format(win, i_episode, win_rate_[i_episode,0])+' ---LOADING: %3f' % (i_episode/MAX_STEP))
...
def test(RL):
    ...
    for i_episode in range(MAX_STEP):
        ...
        if reward == 1:
            win += 1
        win_rate_[i_episode,0] = win/(i_episode+1)*100
        print("WIN: {:03d}/{:03d} ({:.3f}%)".format(win, i_episode, win_rate_[i_episode,0]))
        env.reset()
